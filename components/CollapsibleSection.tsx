  ...
  return (
    <section className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-lg transition-all duration-300 hover:border-slate-700 hover:shadow-pink-500/10">
      <button
        ...
        className="w-full p-6 flex items-center justify-between text-left hover:bg-slate-800/30 transition-colors rounded-t-2xl"
      >
        <h2 className="text-xl font-bold text-white flex items-center">
          {icon && <span className="mr-3 text-pink-500">{icon}</span>}
          {title}
        </h2>
        <ChevronDownIcon 
